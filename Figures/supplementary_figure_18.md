...
F_SITES_5<- ggplot() + 
  geom_point(data=subset(fst_sites,CHROM=="SM_V7_3" & POS>0 & POS<2000000), 
...
XPEHH_SITES_5 <- ggplot() + 
  geom_point(data=subset(xpehh_allvsko,CHROM=="3" & pos>0 & pos<2000000), 
...
ihs_noko$COLOR <- ifelse(ihs_noko$V1=="2" & ihs_noko$V3>332001 & ihs_noko$V3<498001, "C","D")
	
IHS_SITES_5 <- ggplot() + 
  geom_point(data=subset(ihs_noko,V1=="3" & V3>0 & V3<2000000), 
...
IHS_SITES_5B <- ggplot() + 
  geom_point(data=subset(ihs_ko,V1=="3" & V3>0 & V3<2000000), 
             aes(x=V3/1000000, y=abs(V8), color=COLOR), size=0.1,se=FALSE) +
  scale_x_continuous(expand=c(0,0), limits=c(0, 2)) +
  scale_y_continuous(expand=c(0,0), limits=c(0, 10)) + theme_bw() + PCA_theme +
...
```{r}
PI_BOTH_5 <- ggplot() + 
  geom_point(data=subset(pi_site_both,CHROM=="SM_V7_3" & POS>0 & POS<2000000), 
...
```{r}
cov_site_5 <- ggplot() + 
  geom_pointrange(data=subset(df.cov.summary,V1.x=="SM_V7_3" & V2>0 & V2<2000000), 
...
## Plot genes
```{r}
GENES_SITES_5 <- ggplot(data=subset(gff,V1=="SM_V7_3" & V2>0 & V2<2000000), 
...
## Merge plots
```{r}
plot_grid(F_SITES_5,XPEHH_SITES_5,IHS_SITES_5,IHS_SITES_5B,PI_BOTH_5,cov_site_5,GENES_SITES_5, ncol=1, align="v")
```
