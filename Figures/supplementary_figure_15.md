# Supplementary Figure 15: Selection statistics over candidate regions of selection
## Load themes and required packages
```{r}
...
fst_sites$COLOR <- ifelse(fst_sites$CHROM=="SM_V7_2" & fst_sites$POS>3436001 & fst_sites$POS<4806001, "C","D")

F_SITES_3 <- ggplot() + 
  geom_point(data=subset(fst_sites,CHROM=="SM_V7_2" & POS>3000000 & POS<5000000), 
...
xpehh_allvsko$COLOR <- ifelse(xpehh_allvsko$CHROM=="2" & xpehh_allvsko$pos>2624001 & xpehh_allvsko$pos<2756001, "C",
                              ifelse(xpehh_allvsko$CHROM=="2" & xpehh_allvsko$pos>3434001 & xpehh_allvsko$pos<4722001,"C","D"))

XPEHH_SITES_3 <- ggplot() + 
  geom_point(data=subset(xpehh_allvsko,CHROM=="2" & pos>3000000 & pos<5000000), 
...
IHS_SITES_3 <- ggplot() + 
  geom_point(data=subset(ihs_noko,V1=="2" & V3>3000000 & V3<5000000), 
...
IHS_SITES_3B <- ggplot() + 
  geom_point(data=subset(ihs_ko,V1=="2" & V3>3000000 & V3<5000000), 
             aes(x=V3/1000000, y=abs(V8), color=COLOR), size=0.1,se=FALSE) +
  scale_x_continuous(expand=c(0,0), limits=c(3, 5)) +
  scale_y_continuous(expand=c(0,0), limits=c(0, 10)) + theme_bw() + PCA_theme +
...
```{r}
PI_BOTH_3 <- ggplot() + 
  geom_point(data=subset(pi_site_both,CHROM=="SM_V7_2" & POS>3000000 & POS<5000000), 
...
```{r}
cov_site_3 <- ggplot() + 
  geom_pointrange(data=subset(df.cov.summary,V1.x=="SM_V7_2" & V2>3000000 & V2<5000000), 
...
## Plot genes
```{r}
GENES_SITES_3 <- ggplot(data=subset(gff,V1=="SM_V7_2" & V2>3000000 & V2<5000000), 
...
## Merge plots
```{r}
plot_grid(F_SITES_3,XPEHH_SITES_3,IHS_SITES_3,IHS_SITES_3B,PI_BOTH_3,cov_site_3,GENES_SITES_3, ncol=1, align="v")
```
